...
public class RelationServiceImpl implements RelationService {
	...
	public List<Relation> findMyFriends(UserNomade nomade){
		...
		return relationRepository.findByFriendStateAndNomadeOrFriendStateAndNomade2(FriendState.AMI, nomade, FriendState.AMI, nomade);
	}
	
public List<Relation> findMyFriendUnique(UserNomade nomade, UserNomade nomade1){
		
		List<Relation> list = relationRepository.findByNomadeAndNomade2(nomade, nomade1);
		if(list!=null && list.size()>0){
			System.out.print("list: "+list);
			return list;
		}
		List<Relation> list1 = relationRepository.findByNomadeAndNomade2(nomade1, nomade);
	...
	public List<Relation> findReceivedDemand(UserNomade nomade){
		...
		return relationRepository.findByNomade2AndFriendState(nomade, FriendState.EN_ATTENTE);
	}
